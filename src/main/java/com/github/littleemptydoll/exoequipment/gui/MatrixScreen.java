...
public class MatrixScreen extends AbstractContainerScreen<MatrixMenu> {
    ...
    private void drawVerticalRepeat(GuiGraphics guiGraphics, int x, int y, int height,
                                    int u, int v, int sourceWidth, int sourceHeight) {
    ...
    private void renderMatrixGrid(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (previewOrigin != null && draggedModule == null
            ...
            renderCarriedModulePreview(guiGraphics, gridX, gridY,
                    previewOrigin[0], previewOrigin[1]);
        }
    }
    ...
    private void renderCarriedModulePreview(GuiGraphics guiGraphics, int gridX, int gridY,
        ...
        ModuleSize size = MatrixOperations.getRotatedSize(
                moduleItem.getDefinition().size(), carriedModuleRotation);
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        guiGraphics.drawString(font,
                Component.translatable("gui.exoequipment.matrix_size",
                        menu.getMatrixWidth(), menu.getMatrixHeight()),
    ...
    private void drawCharacteristicsButton(GuiGraphics guiGraphics) {
        int x = imageWidth - 20;
        guiGraphics.blit(CONTROLS_TEXTURE, x, BUTTON_Y,
                characteristicsHovered ? BUTTON_HOVER_U : BUTTON_NORMAL_U,
    ...
    private int[] clampOrigin(int originX, int originY, ModuleSize size) {
        ...
        return new int[]{Math.max(0, Math.min(originX, maxX)),
    ...
    private boolean isPendingMoveApplied() {
        ...
        return target != null && target.id().equals(draggedModule.id())
                && target.x() == pendingMoveX && target.y() == pendingMoveY
